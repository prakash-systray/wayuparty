...
<script>
 function getOrdersList(bottleServiceUUID){
	 ...
	 $("#ordersLoaderDiv").attr('style','position:absolute; width:100%; height:350px; background-color:rgba(255,255,255,0.8); top:0px; left:0px; z-index:100;background-image:url("/resources/img/loader.gif"); background-position:center; background-repeat:no-repeat; background-size:75px;');
	  var result = "";
	      ...
	      "ajax": $.fn.dataTable.pipeline( {
	        ...
	        "columnDefs": [ {
		              ...
		              "render": function ( data, type, opt, meta ) {
		            	  ...
		            	  result = result +"<td>";
		                  result = result +"<div class='width150px'>";
	                	  result = result +"<div class='tbldata-dotdot-big pull-left' title='"+opt.vendorOrderStatus+"'>"+opt.vendorOrderStatus;
	                	  result = result +"</div>"
	                	  result = result +"</td>";
	                	  return result;
		              }
		              ...
		              "render": function ( data, type, opt, meta ) {
		            	    ...
		            	    if(opt.vendorOrderStatus == 'Pending For Approval'){
		            	    	    result = result +'<td>';
				            	    result = result +'<div class="tblminwidth-90">';
				            	    result = result +'<ul class="list-unstyled list-inline  clearfix">';
				            	    result = result +'<li style="float: left; margin-left: 15px;"><a href="#" data-toggle="modal" data-target="#right-slide-modal" onclick="getOrderDetails(\''+opt.orderUUID+'\')"><i class="fas fa-eye cursorPointer text-info" title="Details"></i></a></li>';
				            	    result = result +'<li style="float: left; margin-left: 15px;"><a href="#"><i class="fas fa-user-plus cursorPointer text-success" title="Accept" onclick="acceptOrder(\''+opt.orderUUID+'\',\''+bottleServiceUUID+'\')"></i></a></li>';
				            	    result = result +'<li style="float: left; margin-left: 15px;"><a href="#"><i class="fas fa-user-times cursorPointer text-danger" title="Reject"></i></a></li>';
				            	    result = result +'</ul>';
				            	    result = result +'</div>';
				            	    result = result +'</td>'; 
		            	    }else{
				            	    ...
				            	    result = result +'<div class="tblminwidth-90">';
				            	    result = result +'<ul class="list-unstyled list-inline  clearfix">';
				            	    result = result +'<li style="float: left; margin-left: 15px;"><a href="#">User Arrived</a></li>';
				            	    result = result +'</ul>';
				            	    result = result +'</div>';
				            	    result = result +'</td>'; 
		            	    }
