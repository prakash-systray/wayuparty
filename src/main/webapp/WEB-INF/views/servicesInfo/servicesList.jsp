	    						 ...
	    						 result = result +'<ul class="nav nav-tabs vendor-cart" role="tablist">';
	    						  for (var i=0; i<response.object.servicesList.length; i++)
	    			        		  ...
	    			        		  if(service.serviceName != 'Events'){
		    			        		  ...
		    			        		  result = result +'<a class="nav-link border-pastel-gold '+active+'" data-value="'+service.serviceDisplayName+'" data-toggle="tab" href="#service"  onclick="getServiceCategories(\''+service.serviceUUID+'\',\''+service.isEntryRatioEnabled+'\')" role="tab" aria-selected="true" ><img src="'+service.serviceImage+'"></img></a><span><strong>'+service.serviceName+'</strong></span>';
		    			        		  result = result +'</li>';
	    			        		  }
			     ...
			     success :function(resultData) {
			    	 ...
			    	 if(resultData.data.length > 0){
						 ...
						 for (var i=0; i<resultData.data.length; i++)
							  ...
							  var servicePrice = '';
							  var quantityButtons = '';
							  if(opt.serviceName == 'Deals and Offers' || opt.serviceName == 'Surprise' || opt.serviceName == 'Schedule a table' || opt.serviceName == 'Packages'){
